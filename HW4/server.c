...
typedef struct
{
    ...
    int hasClient; /* 0 no client bound yet, 1 bound */
} Session;
...
static void loadAccounts(void)
{
    ...
    while (fgets(line, sizeof(line), fp))
    {
        ...
        if (count >= 3)
        { /* support forms: user pass status | or user pass email homepage status */
            Account acc;
...
static int findAccountByUsername(const char *username)
{
    for (int i = 0; i < numAccounts; ++i)
    {
        if (stringsAreEqual(accounts[i].username, username))
            return i;
    }
    return -1;
}
...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        int bytes = recvfrom(serverSock, buffer, BUFFER_SIZE - 1, 0, (struct sockaddr *)&clientAddr, &clientLen);
        if (bytes < 0)
        {
            perror("recvfrom");
            continue;
        ...
        /* Track a single client per session; if a new one connects, reset session state */
        if (!session.hasClient || memcmp(&clientAddr, &session.clientAddr, sizeof(struct sockaddr_in)) != 0)
        {
            session.stage = STAGE_WAIT_USERNAME;
            session.failCount = 0;
            session.currentUserIndex = -1;
        ...
        else if (session.stage == STAGE_LOGGED_IN)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    accounts[session.currentUserIndex].password[sizeof(accounts[session.currentUserIndex].password) - 1] = '\0';
                    persistAccounts();
        ...
        if (!reply)
            reply = "";
        sendto(serverSock, reply, (int)strlen(reply), 0, (struct sockaddr *)&clientAddr, clientLen);
    }
